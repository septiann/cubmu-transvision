...
const CategoryDesktop = ({ active, handleClick }) => {
  ...
  return (
    <div className={`${styles.paddingX} w-full mt-8 text-white md:flex xs:hidden lg:px-56`}>
      <div
        className={`bg-[#1E1F26] flex items-center justify-center py-5 ${active === null ? "text-white" : "text-[#B1B2B3]"} font-mulish font-[600] w-[160px]`}
      ...
      {dataCategory.result?.map((item, index) => (
        <div
          key={item.categoryId}
          className={`bg-[#1E1F26] flex items-center justify-center py-5 ${active === item.categoryId ? "text-white" : "text-[#B1B2B3]"} font-mulish font-[600] w-[160px]`}
